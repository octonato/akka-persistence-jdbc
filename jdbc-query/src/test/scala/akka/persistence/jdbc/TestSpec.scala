...
abstract class TestSpec(override val config: Config) extends SimpleSpec with MaterializerSpec with DropCreate with BeforeAndAfterAll {
  ...
  val cfg = system.settings.config.getConfig("jdbc-read-journal")
  val readJournalConfig = new ReadJournalConfig(cfg)
  val db = SlickDatabase.forConfig(cfg, readJournalConfig.slickConfiguration)
